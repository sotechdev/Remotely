<#
.SYNOPSIS
   Configures IIS and installs the SODesk server.
.COPYRIGHT
   Copyright ©  2020 Translucency Software.  All rights reserved.
#>
param (
    # The host name (excluding scheme) for the server that will run SODesk.
    [Parameter(Mandatory=$True)]
	[string]$HostName,
     # The name to use for the IIS Application Pool for the SODesk site.
    [Parameter(Mandatory=$True)]
	[string]$AppPoolName,
     # The name to use for the IIS site.
    [Parameter(Mandatory=$True)]
	[string]$SiteName,
    # The folder path where the SODesk server files are located.
    [Parameter(Mandatory=$True)]
    [string]$SitePath,
...
$Host.UI.RawUI.WindowTitle = "SODesk Setup"
Clear-Host
...
if ($OS.Name.ToLower().Contains("home") -or $OS.Caption.ToLower().Contains("home")) {
    Wrap-Host
    Wrap-Host "Error: Windows Home version does not have the necessary features to run SODesk." -ForegroundColor Red
    Do-Pause
    return
}
...
Wrap-Host
Wrap-Host "**********************************"
Wrap-Host "         SODesk Setup" -ForegroundColor Cyan
Wrap-Host "**********************************"
Wrap-Host
Wrap-Host "Hello, and thank you for trying out SODesk!" -ForegroundColor Green
Wrap-Host
Wrap-Host "This setup script will create an IIS site and install SODesk on this machine." -ForegroundColor Green
Wrap-Host
Do-Pause
Clear-Host
