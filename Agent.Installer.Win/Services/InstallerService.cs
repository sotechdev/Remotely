...
namespace SODesk.Agent.Installer.Win.Services
{
    public class InstallerService
    {
        ...
        private string InstallPath => Path.Combine(Path.GetPathRoot(Environment.SystemDirectory), "Program Files", "SODesk");
            ...
            bool createSupportShortcut)
        {
            try
            {
                ...
                await DownloadSODeskAgent(serverUrl);

                FileIO.WriteAllText(Path.Combine(InstallPath, "ConnectionInfo.json"), Serializer.Serialize(connectionInfo));

                FileIO.Copy(Assembly.GetExecutingAssembly().Location, Path.Combine(InstallPath, "SODesk_Installer.exe"));
        ...
        public async Task<bool> Uninstall()
        {
            try
            {
                ...
                ProcessEx.StartHidden("cmd.exe", "/c sc delete SODesk_Service").WaitForExit();
                ...
                ProcessEx.StartHidden("netsh", "advfirewall firewall delete rule name=\"SODesk Desktop Unattended\"").WaitForExit();

                GetRegistryBaseKey().DeleteSubKeyTree(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SODesk", false);
        ...
        {
            var desktopExePath = Path.Combine(InstallPath, "Desktop", "SODesk_Desktop.exe");
            ProcessEx.StartHidden("netsh", "advfirewall firewall delete rule name=\"SODesk Desktop Unattended\"").WaitForExit();
            ProcessEx.StartHidden("netsh", $"advfirewall firewall add rule name=\"SODesk Desktop Unattended\" program=\"{desktopExePath}\" protocol=any dir=in enable=yes action=allow description=\"The agent that allows screen sharing and remote control for SODesk.\"").WaitForExit();
        }
        ...
        private void BackupDirectory()
        {
            if (Directory.Exists(InstallPath))
            {
                ...
                var backupPath = Path.Combine(Path.GetTempPath(), "SODesk_Backup.zip");
        ...
        private void CreateSupportShortcut(string serverUrl, string deviceUuid, bool createSupportShortcut)
        {
            ...
            shortcut.IconLocation = Path.Combine(InstallPath, "SODesk_Agent.exe");
        ...
        {
            var version = FileVersionInfo.GetVersionInfo(Path.Combine(InstallPath, "SODesk_Agent.exe"));
            var baseKey = GetRegistryBaseKey();

            var remotelyKey = baseKey.CreateSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SODesk", true);
            remotelyKey.SetValue("DisplayIcon", Path.Combine(InstallPath, "SODesk_Agent.exe"));
            remotelyKey.SetValue("DisplayName", "SODesk");
            remotelyKey.SetValue("DisplayVersion", version.FileVersion);
            remotelyKey.SetValue("InstallDate", DateTime.Now.ToShortDateString());
            remotelyKey.SetValue("Publisher", "SOTech Sistemas");
            ...
            remotelyKey.SetValue("UninstallString", Path.Combine(InstallPath, "SODesk_Installer.exe -uninstall -quiet"));
            remotelyKey.SetValue("QuietUninstallString", Path.Combine(InstallPath, "SODesk_Installer.exe -uninstall -quiet"));
        }

        private async Task DownloadSODeskAgent(string serverUrl)
        {
            var targetFile = Path.Combine(Path.GetTempPath(), $"SODesk-Agent.zip");
            ...
            {
                ProgressMessageChanged.Invoke(this, "Downloading SODesk agent.");
                using (var client = new WebClient())
                {
                    ...
                    await client.DownloadFileTaskAsync($"{serverUrl}/Content/SODesk-Win10-{Platform}.zip", targetFile);
                }
            }

            ProgressMessageChanged.Invoke(this, "Extracting SODesk files.");
            ProgressValueChanged?.Invoke(this, 0);

            var tempDir = Path.Combine(Path.GetTempPath(), "SODeskUpdate");
            ...
            var wr = WebRequest.CreateHttp($"{serverUrl}/Content/SODesk-Win10-{Platform}.zip");
            wr.Method = "Head";
        ...
        private void InstallService()
        {
            ...
            ProgressMessageChanged?.Invoke(this, "Installing SODesk service.");
            var serv = ServiceController.GetServices().FirstOrDefault(ser => ser.ServiceName == "SODesk_Service");
            if (serv == null)
            {
                var command = new string[] { "/assemblypath=" + Path.Combine(InstallPath, "SODesk_Agent.exe") };
                var context = new InstallContext("", command);
                var serviceInstaller = new ServiceInstaller()
                {
                    Context = context,
                    DisplayName = "SODesk Service",
                    Description = "Background service that maintains a connection to the SODesk server.  The service is used for remote support and maintenance by this computer's administrators.",
                    ServiceName = "SODesk_Service",
                ...
                serv = ServiceController.GetServices().FirstOrDefault(ser => ser.ServiceName == "SODesk_Service");

                ProcessEx.StartHidden("cmd.exe", "/c sc.exe failure \"SODesk_Service\" reset= 5 actions= restart/5000");
            }
        ...
        {
            ProgressMessageChanged?.Invoke(this, "Stopping SODesk processes.");
            var procs = Process.GetProcessesByName("SODesk_Agent").Concat(Process.GetProcessesByName("SODesk_Desktop"));
        ...
        private void StopService()
        {
            try
            {
                var remotelyService = ServiceController.GetServices().FirstOrDefault(x => x.ServiceName == "SODesk_Service");
                if (remotelyService != null)
                {
                    Logger.Write("Stopping existing SODesk service.");
                    ProgressMessageChanged?.Invoke(this, "Stopping existing SODesk service.");
