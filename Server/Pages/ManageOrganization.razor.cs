...
using SODesk.Server.Components;
using SODesk.Server.Components.ModalContents;
...
namespace SODesk.Server.Pages
{
    public partial class ManageOrganization : AuthComponentBase
    {
        ...
        private readonly List<SODeskUser> _orgUsers = new();
        ...
        [Inject]
        private UserManager<SODeskUser> UserManager { get; set; }
        ...
        private async Task DeleteUser(SODeskUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task EditDeviceGroups(SODeskUser user)
        {
        ...
        private async Task ResetPassword(SODeskUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task SendInvite()
        {
            ...
            else
            {
                ...
                var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser, "Invitation to Organization in SODesk",
                        $@"<img src='{NavManager.BaseUri}images/SODesk_Logo.png'/>
                            <br><br>
                            Hello!
                            <br><br>
                            You've been invited to join an organization in SODesk.
                            <br><br>
        ...
        private void SetUserIsAdmin(ChangeEventArgs args, SODeskUser orgUser)
        {
            if (!User.IsAdministrator)
            {
                return;
