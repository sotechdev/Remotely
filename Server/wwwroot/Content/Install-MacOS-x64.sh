...
if [ -f "/usr/local/bin/SODesk/ConnectionInfo.json" ]; then
    SavedGUID=`su - $Owner -c "cat '/usr/local/bin/SODesk/ConnectionInfo.json' | jq -r '.DeviceID'"`
...
if [ -z "$UpdatePackagePath" ]; then
    ...
    curl $HostName/Content/SODesk-MacOS-x64.zip --output /usr/local/bin/SODesk/SODesk-MacOS-x64.zip
else
    echo  "Copying install files..." >> /tmp/SODesk_Install.log
    cp "$UpdatePackagePath" /usr/local/bin/SODesk/SODesk-MacOS-x64.zip
    rm -f "$UpdatePackagePath"
fi

unzip -o ./SODesk-MacOS-x64.zip
rm -f ./SODesk-MacOS-x64.zip
...
curl --head $HostName/Content/SODesk-MacOS-x64.zip | grep -i "etag" | cut -d' ' -f 2 > ./etag.txt
